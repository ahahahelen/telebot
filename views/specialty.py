...
class DIService(containers.DeclarativeContainer):
    db_session = providers.Object()


def first_msg(message):
    bot = DIBot.di_bot()
    session = DIService.db_session()
    spec = session.query(Bachelor).filter_by(id_spec=1).first()
    print(Bachelor)
...
def pages_keyboard(numb):
    ...
    """
    session = DIService.db_session()
    numb_fields = session.query(Bachelor).count()
...
def pages(c):
    ...
    if c.data == 'help':
        ...
        spec = session.query(Bachelor).filter_by(id_spec=c.data[3:]).first()
